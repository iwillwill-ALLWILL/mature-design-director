...
def main() -> None:
    ...
    if args.safe_margin_x < 0 or args.safe_margin_y < 0:
        raise SystemExit("safe margins cannot be negative")
    if args.safe_margin_x * 2 >= args.cell_width or args.safe_margin_y * 2 >= args.cell_height:
        raise SystemExit("safe margins must leave a positive drawable area inside every cell")
